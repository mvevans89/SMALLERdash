...
#'
#' @importFrom shiny NS tagList
mod_cases_time_plotly_ui <- function(id){
  ns <- NS(id)
  fluidRow(
    #inputs
    column(3,
...
#' @noRd
mod_cases_time_plotly_server <- function(id){
  moduleServer( id, function(input, output, session){
    ns <- session$ns
    output$plot <- renderPlotly({plot_cases_time_plotly(csbSelect = input$csb)})
  })
}
